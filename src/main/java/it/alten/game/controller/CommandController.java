...
//            }
        if (command.toLowerCase().startsWith("go")) {
            String directionToTake = command.substring(2).trim();
            if (directionToTake.equalsIgnoreCase("north")) {
                gameController.changeRoom(AdjacentRooms.NORTH);
                look();
            } else if (directionToTake.equalsIgnoreCase("south")) {
                gameController.changeRoom(AdjacentRooms.SOUTH);
                look();
            } else if (directionToTake.equalsIgnoreCase("east")) {
                gameController.changeRoom(AdjacentRooms.EAST);
                look();
            } else if (directionToTake.equalsIgnoreCase("west")) {
                gameController.changeRoom(AdjacentRooms.WEST);
                look();
            } else {
                System.out.println("Coglione non c'è una porta su quel lato");
